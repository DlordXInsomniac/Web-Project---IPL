lsgData.forEach((element, i ) => {
    ...
    card.classList = 'card';
    card.dataset.name = lsgData[i].id;
    const preview = document.createElement('div');
    preview.classList = 'preview';
    preview.dataset.target = lsgData[i].id;

    
    const playerCard = `
    
    <div class="player-pic"><img src=${lsgData[i].pic}></div>
    <div class="details">
        <h2>${lsgData[i].PlayerName}</h2>
        <h5>Team : ${lsgData[i].From}</h5>
        <h5>Price : ${lsgData[i].Price}</h5>
        <h5>Playing Status : ${lsgData[i].isPlaying}</h5>
        <h5>Role : ${lsgData[i].description}</h5>
    </div>
    `;
    ...
    <img class="close" src="https://img.icons8.com/material-outlined/24/000000/delete-sign.png"/>
        <div class="player-img">
            <img src=${lsgData[i].pic}>
        </div>
        <div class="pl-details">
            <p>${lsgData[i].PlayerName}</p>
            <h4>Team : ${lsgData[i].From}</h4>
            <h4>Price : ${lsgData[i].Price} </h4>
            <h4>Role : ${lsgData[i].description}</h4>
        </div>
</div>
`;
